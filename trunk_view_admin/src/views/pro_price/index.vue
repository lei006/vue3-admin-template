  ...
  <div class="layout-container">
    <div class="layout-container-form flex space-between">
      <div class="layout-container-form-handle">
        ...
        <el-button type="primary" :icon="Plus" :disabled="!project_id" @click="handleAdd">{{
      ...
      >
        ...
        <el-table-column prop="project_id" label="项目" align="center" width="160">
          <template #default="scope">
                <div style="line-height: 32px;"> {{ getProjectName(scope.row.project_id)}} </div>
          </template>
        </el-table-column>
...
import apiProPriceOption from "@/api/pro/price";
...
const onBtnAdd = () => {
  ...
  addForm.project_id = project_id.value;

  apiProPriceOption.AddOne(addForm).then(res => {
...
const getTableData = (init: Boolean) => {
  ...
  apiProPriceOption.GetPage({project_id: project_id.value}).then((res) => {
...
const handleDel = (data: object[]) => {
    ...
    apiProPriceOption.DeleteMany(ids).then((res) => {
...
const handleUpdateStatus = (row, field, data) => {
    ...
    row.loading = true
    apiProPriceOption.PatchOne(row.id,field, data ). then(res => {
        if (res.code === 200) {
...
const handleFieldEdit = (row: object, field :string) => {
    ...
    ElMessageBox.prompt('', '修改:' + field, {inputValue: row[field], confirmButtonText: '确定',cancelButtonText: '取消'}).then(({ value }) => {
      apiProPriceOption.PatchOne(row.id, field, value).then(res => {
            if (res.code === 200) {
                row[field] = res.data[field];
            } else {
                ElMessage({type: 'error',message: '操作失败'})
